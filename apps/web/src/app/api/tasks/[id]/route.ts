...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: task }, { data: documents }, { data: history }] = await Promise.all([
    ...
    supabaseAdmin.from("task_documents").select("*").eq("task_id", id).order("created_at"),
    supabaseAdmin.from("task_history").select("*").eq("task_id", id).order("changed_at"),
  ]);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: current } = await supabaseAdmin.from("tasks").select("status").eq("id", id).single();
